...
func main() {
	ca := &x509.Certificate{
		...
		Subject: pkix.Name{
			...
			Organization:       []string{"Kool"},
			OrganizationalUnit: []string{"KoolU"},
		},
		NotBefore:             time.Now(),
		NotAfter:              time.Now().AddDate(1, 0, 0),
		SubjectKeyId:          []byte{6, 3, 7, 9, 1},
	...
	cert2 := &x509.Certificate{
		...
		Subject: pkix.Name{
			Country:            []string{"China"},
			Organization:       []string{"Kool2"},
			OrganizationalUnit: []string{"KoolU2"},
		},
		NotBefore:    time.Now(),
		NotAfter:     time.Now().AddDate(1, 0, 0),
		SubjectKeyId: []byte{9, 3, 1, 8, 7},
		ExtKeyUsage:  []x509.ExtKeyUsage{x509.ExtKeyUsageClientAuth, x509.ExtKeyUsageServerAuth},
		KeyUsage:     x509.KeyUsageDigitalSignature | x509.KeyUsageCertSign,
	}
